

export const articleTooltips = {
  articleSubtitle : "Additional title information",
  alternateTitle : "Original language title for translated works only",
  alternateSubtitle : "Original language subtitle for translated works only",
  orcid : "ORCID identifier (https://orcid.org)",
  groupAuthorName : "Organization name contributing to article",
  groupAuthorRole : "Organization role contributing to article",
  printDate : "Date article was published in print",
  onlineDate : "Date article was published online",
  locationId : "Article number, ID, or e-location ID if a page number is not used",
  funderId : "Funder ID from Funder Registry (https://www.crossref.org/services/funder-registry/)",
  grantNumber : "Grant or other award number",
  licenseUrl : "Link to article license",
  relationType : "Type of relationship between the article and another content item (https://support.crossref.org/hc/en-us/articles/214357426-Relationships-between-DOIs-and-other-objects)",
  archiveLocation : "Designated archiving organization for your content",
  similarityCheckURL : "Crawler-friendly link to index content for Similarity Check",
  language : "Primary language for your article",
  publicationHistory: 'Complete if you want this DOIs publication history to show in the "More information" section of the Crossmark box.',
  peerReview: "Complete if you want this information to display in the Crossmark box",
  copyrightLicensing: "License URLs should be deposited in the main licenses section. This space is for any additional information or context.",
  other: "Complete if you want this information to display in the Crossmark box",
  update: "Only complete this section if the DOI you are depositing is an update (e.g. correction or retraction notice) to another DOI"
}
